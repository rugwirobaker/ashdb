...
pub struct Table {
    ...
    offset: u64, // Current file offset (write mode only)
    block_cache: Option<Arc<Mutex<Cache<u64, Arc<Block>>>>>, // For read mode
}

impl Table {
    /// Opens a table in write mode
    pub fn create(path: &str) -> Result<Self> {
        let file = File::create(path)?;
        Ok(Self {
            file,
            mode: TableMode::Write,
            index: Index::new(),
            offset: 0,
            block_cache: None,
        })
    ...
    pub fn open(path: &str) -> Result<Self> {
        ...
        Ok(Self {
            ...
            index: index,
            offset: 0,
            block_cache: Some(Arc::new(Mutex::new(Cache::new(1000, None)))),
        })
    }

    /// Adds a block to the table (write mode)
    pub fn add_block(&mut self, block_data: &[u8], first_key: Vec<u8>) -> Result<()> {
        match self.mode {
            ...
            TableMode::Read => Err(Error::InvalidOperation(
                "Cannot add block in read mode".to_string(),
            )),
        }
    }

    /// Finalizes the table (write mode)
    pub fn finish(&mut self) -> Result<()> {
        match self.mode {
            TableMode::Write => {
                // indexes might get big, a future optimization could not to copy it
                // and instead put it behind a sm
            ...
            TableMode::Read => Err(Error::InvalidOperation(
                "Cannot finalize in read mode".to_string(),
            )),
        }
    }

    /// Gets a value for a key (read mode)
    pub fn get(&mut self, key: &[u8]) -> Result<Option<Vec<u8>>> {
        match self.mode {
            TableMode::Read => {
                if let Some(entry) = self.index.find(key) {
                    let block_size = if entry.index + 1 < self.index.len() {
                        ...
                        self.offset - entry.offset // Use index offset for last block
                    };
            ...
            TableMode::Write => Err(Error::InvalidOperation(
                "Cannot get value in write mode".to_string(),
            )),
        }
    }

    /// Scans the entire table (read mode)
    pub fn scan(&mut self) -> Result<ScanIterator> {
        match self.mode {
            TableMode::Read => Ok(ScanIterator::new(self)),
            TableMode::Write => Err(Error::InvalidOperation(
                "Cannot scan in write mode".to_string(),
            )),
        }
    }

    /// Reads a block from the file (read mode)
    fn read_block(&mut self, offset: u64, size: u64) -> Result<Arc<Block>> {
        if let Some(cache) = &self.block_cache {
            if let Some(block_arc) = cache.lock().unwrap().get(&offset) {
        ...
        if let Some(cache) = &self.block_cache {
            cache.lock().unwrap().insert(offset, block.clone());
        }
...
/// Iterator for scanning the table
...
impl<'a> ScanIterator<'a> {
    ...
    fn next_block(&mut self) -> Result<()> {
        if self.current_block_index >= self.table.index.len() {
            self.current_block_iter = None;
            return Ok(());
        }

        let block_offset = self.table.index.get(self.current_block_index).unwrap();
        let block_size = if self.current_block_index + 1 < self.table.index.len() {
            self.table.index.get(self.current_block_index + 1).unwrap() - block_offset
        } else {
            self.table.offset - block_offset
        };
        let block = self.table.read_block(block_offset, block_size)?;
